...
public class DogsAlarm implements Alarm {
    ...
    public void alarmTurnOn(PinEvent pinEvent) {
        if(logger != null) {
            logger.logMessage(Severity.SEVERE, new EventSource("Dogs alarm", this.getClass()), "Dogs alarm turned on");
        }
        _isTurnedOn = true;
    }
    ...
    public void alarmTurnOff(PinEvent pinEvent) {
        if(logger != null) {
            logger.logMessage(Severity.NORMAL, new EventSource("Dogs alarm", this.getClass()), "Dogs alarm turned off");
        }
        _isTurnedOn = false;
    }
