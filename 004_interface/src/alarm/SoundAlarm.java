...
public class SoundAlarm implements Alarm {
    ...
    public void alarmTurnOn(PinEvent pinEvent) {
        if(logger != null) {
            logger.logMessage(Severity.SEVERE, new EventSource("Sound alarm", this.getClass()), "Sound alarm turned on");
        }
        _isTurnedOn = true;
    }
    ...
    public void alarmTurnOff(PinEvent pinEvent) {
        if(logger != null) {
            logger.logMessage(Severity.NORMAL, new EventSource("Sound alarm", this.getClass()), "Sound alarm turned off");
        }
        _isTurnedOn = false;
    }
